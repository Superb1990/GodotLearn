...
![选择碰撞形状](img/img_dodge/5.png)    

----
> 2. 在2d视图去，根据情况框选碰撞区域  

![选择碰撞形状](img/img_dodge/6.png)   
...
![代码部分](img/img_dodge/10.png)   

> 2. 根据游戏进程设置当前player的可见性
初始化时，隐藏player，游戏开始时，再显示出来，当和怪物发生碰撞时，再隐藏（同时发出被怪物击打到的**信号**）  

![代码部分](img/img_dodge/11.png)![代码部分](img/img_dodge/12.png)      
...
![怪物结构](img/img_dodge/13.png)!

### 刚体Mob
> 需要将刚体的中立设置为0，否则将造成绑定该刚体的物体一直下落  

![怪物结构](img/img_dodge/14.png)  

### 物体动画Mob  
> 同样需要设置序列帧动画集合`Sprite Frame`   

![怪物结构](img/img_dodge/15.png)  ![怪物结构](img/img_dodge/16.png)  

### 自身碰撞区域Mob  
> 设置碰撞形状
根据实际情况可调整**旋转角度**以适应player对应图像的方位  

![怪物结构](img/img_dodge/17.png)  ![怪物结构](img/img_dodge/18.png)    

### 可见性矩形Mob
> 一般用于触发信号使用，当怪物离开指定区域后隐藏  

![怪物结构](img/img_dodge/19.png)  

### 代码部分-挂载在刚体上Mob
![怪物结构](img/img_dodge/20.png)  
...
![HUD](img/img_dodge/21.png)  
### 文本-分数与信息显示HUD
> 设置好位置与字体即可  

![HUD](img/img_dodge/22.png)  ![HUD](img/img_dodge/23.png)  

### 按钮与时间控件-设置好连接信号-HUD
![HUD](img/img_dodge/24.png)  ![HUD](img/img_dodge/25.png)  

### 代码部分-挂载在CanvasLayer上-HUD  
⭐⭐⭐
![HUD](img/img_dodge/26.png)
...
![Main主界面](img/img_dodge/27.png)

### 单色矩形-背景颜色Main
> 设置背景颜色与长宽  

![Main主界面](img/img_dodge/28.png)  

### Player场景Main
> 设置图层优先级为最高，让player始终在所有图层的顶部 
在这里设置为 10   

![Main主界面](img/img_dodge/29.png)  

> 将player挂载脚本中定义的hit信号（player与怪物碰撞时发出）
body_entered信号触发后会隐藏player节点并发出 hit 信号  

![Main主界面](img/img_dodge/30.png)  

### 时间控件Main
> 1. 怪物出现时间控件MobTimer
设置 怪物出生 延时  

![Main主界面](img/img_dodge/31.png)  

> 2. 分数增加时间控件ScoreTimer
无需设置  

![Main主界面](img/img_dodge/32.png)  

> 3. 游戏开始时间控件StartTimer
设置 游戏开始延时、自动重启 

![Main主界面](img/img_dodge/33.png) 

### 位置标记节点Main
> 定义player出生位置

![Main主界面](img/img_dodge/34.png) 

#### 路径节点Main
> 1. Path2D设置路径途径点位  

![Main主界面](img/img_dodge/35.png) 

> 2. PathFollow2D点取样器，用于显示在path2d路径上 某个点的**向量坐标**

![Main主界面](img/img_dodge/36.png) 

### 实例化HUD场景Main
> 将HUD挂载的代码中的 start_game信号 连接到 Main的脚本中

![Main主界面](img/img_dodge/37.png) 

### 音乐播放控件Main
> 将音乐文件拖入stream中

![Main主界面](img/img_dodge/38.png) 

### 代码部分Main-挂载在Node2D
> 1. 定义怪物场景（导出模式）
需要手动拖入该变量  

![Main主界面](img/img_dodge/39.png) 

> 2. 游戏开始、游戏结束
执行大量操作

![Main主界面](img/img_dodge/40.png) 

> 3. 怪物生成逻辑、更新分数、开始timer倒计时
怪物生成逻辑⭐⭐⭐⭐

![Main主界面](img/img_dodge/41.png) 